<?php $id = $id ?? null; ?>
@if (isset($id))
    {{ Form::model($data, ['route' => ['whydelivery.update', $id], 'method' => 'patch','enctype' => 'multipart/form-data']) }}
@else
    {{ Form::open(['route' => ['whydelivery.store'], 'method' => 'post','enctype' => 'multipart/form-data', 'id' => 'whydelivery_form']) }}
@endif
{!! Form::hidden('type', 'why_choose') !!}
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group col-md-12">
                {{ Form::label('title', __('message.title') . ' <span class="text-danger">*' .  __('message.max_100_character') . '</span>', ['class' => 'form-control-label'], false) }}
                {{ Form::text('title', old('title'), ['placeholder' => __('message.title'), 'class' => 'form-control', 'required', 'maxlength' => '100']) }}
                <span class="text-danger" id = "ajax_form_validation_title"></span>
            </div>
            <div class="form-group col-md-12">
                {{ Form::label('subtitle', __('message.subtitle') . ' <span class="text-danger">*' .  __('message.max_100_character') . '</span>', ['class' => 'form-control-label'], false) }}
                {{ Form::text('subtitle', old('subtitle'), ['placeholder' => __('message.subtitle'), 'class' => 'form-control', 'required', 'maxlength' => '100']) }}
