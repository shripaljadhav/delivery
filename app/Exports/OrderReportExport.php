...
class OrderReportExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithEvents
{
    protected $request;
    protected $counter;
     ...
     public function headings($exportType = 'excel'): array
    {
        ...
        if ($exportType === 'excel') {
            $headings[] = [__('message.order_report') . ($date ? ' : ' . $date : '')];
            $headings[] = [];
            $columnHeadings = [];
